...
void *wss_tls_connect(int *sock , char *host, int port){
	...
	if(tls){
		...
		cacert = (struct _x509_crt *)malloc(sizeof(x509_crt));
		if(!cacert){
			printf("Error: malloc cacert struct fail!");
			goto exit;
		}
		...
		x509_crt_parse(cacert, g_pszCrt, g_iCrtLen);// client   CA buf  and CA len
